# @WindfallSwap/windfallswap-token-lists (beta)

[![Tests](https://github.com/WindfallSwap/windfallswap-token-lists/workflows/Tests/badge.svg)](https://github.com/WindfallSwap/windfallswap-token-lists/actions?query=workflow%3ATests)
[![npm](https://img.shields.io/npm/v/@WindfallSwap/windfallswap-token-lists)](https://unpkg.com/@WindfallSwap/windfallswap-token-lists@latest/)

This package includes a JSON schema for token lists, and TypeScript utilities for working with token lists.

The JSON schema represents the technical specification for a token list which can be used in a dApp interface, such as the Windfall Swap Interface.

## What are token lists?

Windfall Swap Token Lists is a specification for lists of token metadata (e.g. address, decimals, ...) that can be used by any dApp interfaces that needs one or more lists of tokens.
...
Token list JSON must validate against the [JSON schema](https://json-schema.org/) in order to be used in the Windfall Swap Interface.
Tokens on token lists, and token lists themselves, are tagged so that users can easily find tokens.

## JSON Schema $id

The JSON schema ID is [https://windfallswap.com/tokenlist.schema.json](https://windfallswap.com/tokenlist.schema.json)
...
```typescript
import { TokenList, schema } from '@WindfallSwap/windfallswap-token-lists'
...
A snapshot of the Windfall Swap default list encoded as a token list is found in [test/schema/bigexample.tokenlist.json](test/schema/bigexample.tokenlist.json).
